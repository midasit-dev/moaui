...
for (const info of arrInfos) {
	if(info.ComponentVariant === "GuideBoxEmpty") continue;
	totalSringV2.push(`\t\t\t\t\t\${item.type === ItemTypes.${info.ComponentVariant} ? \`\${extractComponentName(All.${info.ComponentVariant})}\` : ""}`);
}
