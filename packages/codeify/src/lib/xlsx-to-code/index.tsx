...
import { Box, Container, Paper, Typography, Grid, Alert, Tabs, Tab, AlertTitle, Button } from '@mui/material';
...
const ExcelToDiv: React.FC = () => {
  ...
  const maxColumns = excelData.reduce((max, row) => Math.max(max, row.length), 0);
  ...
  const onDrop = useCallback((acceptedFiles: File[]) => {
    ...
    reader.onload = async (e: ProgressEvent<FileReader>) => {
      ...
      if (e.target && e.target.result) {
        ...
        const rawJson: ExcelJS.CellValue[][] = [];
        ...
        {excelData.length > 0 && tabIndex === 0 && (
          <Grid container spacing={0} sx={{ marginTop: "20px" }}>
            {excelData.length > 0 && tabIndex === 0 && (
				...
				{/** Code Tab */}
        {excelData.length > 0 && tabIndex === 1 && (() => {
					const tsxString = generateCode(excelData, styles, widthHeights);
					return (
