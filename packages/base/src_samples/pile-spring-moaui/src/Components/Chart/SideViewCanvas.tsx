...
const SideView = () => {
  ...
  const InputDegrees = ExtractNumbers(InputPileData['minorDegree'])
  ...
  const drawInputs = (ctx:any) => {
    ...
    const orginX = canvasSize/2 + height/2*YAmplifyRatio
    for (let i = 0; i < pilecoordinate.length; i++) {
      ...
      ctx.lineTo(orginX - pilecoordinate[i][1]*YAmplifyRatio+Diameter/2, canvasSize*0.1+YEndCoordinate);
      // 우측면
      ctx.moveTo(orginX - pilecoordinate[i][1]*YAmplifyRatio+Diameter/2, canvasSize*0.1+YEndCoordinate);
  ...
  const drawTables = (ctx:any) => {
    ...
    for (let i = 0; i < pileTableData.length; i++) {
      ...
      for (let j = 0; j < pilecoordinate.length; j++) {
        // 윗면
        ctx.beginPath();
        ctx.moveTo(orginX - pilecoordinate[j][1]*YAmplifyRatio-Diameter/2, canvasSize*0.1+YEndCoordinate);
        ctx.lineTo(orginX - pilecoordinate[j][1]*YAmplifyRatio+Diameter/2, canvasSize*0.1+YEndCoordinate);
        // 우측면
        ctx.moveTo(orginX - pilecoordinate[j][1]*YAmplifyRatio+Diameter/2, canvasSize*0.1+YEndCoordinate);
        ctx.lineTo(orginX - pilecoordinate[j][1]*YAmplifyRatio+Diameter/2+Math.tan(Math.PI/180*(ExtractNumbers(pileTableData[i]['minorDegree'])[j]))*pileTableData[i]['pileLength']*YAmplifyRatio, canvasSize*0.1+YEndCoordinate+pileTableData[i]['pileLength']*YAmplifyRatio);
        // 아랫면
        ctx.moveTo(orginX - pilecoordinate[j][1]*YAmplifyRatio+Diameter/2+Math.tan(Math.PI/180*(ExtractNumbers(pileTableData[i]['minorDegree'])[j]))*pileTableData[i]['pileLength']*YAmplifyRatio, canvasSize*0.1+YEndCoordinate+pileTableData[i]['pileLength']*YAmplifyRatio);
        ctx.lineTo(orginX - pilecoordinate[j][1]*YAmplifyRatio-Diameter/2+Math.tan(Math.PI/180*(ExtractNumbers(pileTableData[i]['minorDegree'])[j]))*pileTableData[i]['pileLength']*YAmplifyRatio, canvasSize*0.1+YEndCoordinate+pileTableData[i]['pileLength']*YAmplifyRatio);
        // 좌측면
        ctx.moveTo(orginX - pilecoordinate[j][1]*YAmplifyRatio-Diameter/2+Math.tan(Math.PI/180*(ExtractNumbers(pileTableData[i]['minorDegree'])[j]))*pileTableData[i]['pileLength']*YAmplifyRatio, canvasSize*0.1+YEndCoordinate+pileTableData[i]['pileLength']*YAmplifyRatio);
        ctx.lineTo(orginX - pilecoordinate[j][1]*YAmplifyRatio-Diameter/2, canvasSize*0.1+YEndCoordinate)
  ...
  const drawOutlines = (ctx:any) => {
    ...
    ctx.lineWidth = 1;
    const newWidth = height*YAmplifyRatio
    ...
    const offsetY = 5
    ctx.fillText(`${height*1000}`, x + newWidth / 2, y - dimOffset - offsetY);
  };
