...
const ComponentsScatterPlotGetNodes = () => {
  ...
  const [minMax, setMinMax] = useState<{ minL: number; maxL: number; minR: number; maxR: number }>({
    minL: 0,
    maxL: 0,
    minR: 0,
    maxR: 0,
  ...
  return (
    <GuideBox center spacing={1}>
      ...
      <GuideBox row verCenter spacing={2}>
        ...
        />
        <Check checked={colorful} name={colorful ? "colorful" : "mono"} onChange={() => setColorful(!colorful)} />
      </GuideBox>
      {/* <ScatterPlot
				data={data}
				width={700}
