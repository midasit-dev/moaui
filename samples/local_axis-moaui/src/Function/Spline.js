...
function NaturalCubicSpline(xSeries, ySeries, numSeg) {
	...
	for (let i = 1; i < n; i++) {
		alpha[i] =
			(3 / h[i]) * (ySeries[i + 1] - ySeries[i]) -
			(3 / h[i - 1]) * (ySeries[i] - ySeries[i - 1]);
	}
	//Step3 : Set l0, u0, z0
	l[0] = 1.0;
	u[0] = 0.0;
	...
	for (let i = n - 1; i >= 0; i--) {
		c[i] = z[i] - u[i] * c[i + 1];
		b[i] =
			(ySeries[i + 1] - ySeries[i]) / h[i] - (h[i] * (c[i + 1] + 2 * c[i])) / 3;
		d[i] = (c[i + 1] - c[i]) / (3 * h[i]);
	}
	//Interpolation
	...
	Sx =
		ySeries[n - 1] +
		b[n - 1] * diff +
		c[n - 1] * Math.pow(diff, 2) +
		d[n - 1] * Math.pow(diff, 3);
...
function MonotoneCubicSpline(xSeries, ySeries, numSeg) {
	...
	for (let i = 0; i < n; i++) {
		...
		for (let j = 0; j < numSeg; j++) {
			...
			Sx =
				ySeries[i] +
				c1s[i] * diff +
				c2s[i] * Math.pow(diff, 2) +
				c3s[i] * Math.pow(diff, 3);
	...
	Sx =
		ySeries[n - 1] +
		c1s[n - 1] * diff +
		c2s[n - 1] * Math.pow(diff, 2) +
		c3s[n - 1] * Math.pow(diff, 3);
	devSx =
		c1s[n - 1] + 2 * c2s[n - 1] * diff + 3 * c3s[n - 1] * Math.pow(diff, 2);
	Results[k] = [x, Sx, devSx];
