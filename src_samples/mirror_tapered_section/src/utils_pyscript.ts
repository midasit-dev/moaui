...
export function dbGetTaperedSection(){
	return checkPyScriptReady(() => {
		const py_db_get_tapered_section_func = pyscript.interpreter.globals.get('py_db_read_tapered');
		const result = py_db_get_tapered_section_func();
		return JSON.parse(result);
	});
}

export function dbCreateTaperedSection(item: any){
	return checkPyScriptReady(() => {
		const py_db_create_tapered_section_func = pyscript.interpreter.globals.get('py_db_create_tapered');
		const result = py_db_create_tapered_section_func(JSON.stringify(item));
		return JSON.parse(result);
	});
}