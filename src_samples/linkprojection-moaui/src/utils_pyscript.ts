...
export function getSelected(){
	return checkPyScriptReady(() => {
		const py_get_selected_func = pyscript.interpreter.globals.get('get_selected');
		const result = py_get_selected_func();
		return JSON.parse(result);
	});
}

export function ApplyLink(MasterNode:any, SlaveNode:any, LinkType:any){
	return checkPyScriptReady(() => {
		const py_apply_link_func = pyscript.interpreter.globals.get('ApplyLink');
		const result = py_apply_link_func(JSON.stringify(MasterNode), JSON.stringify(SlaveNode), LinkType);
		return JSON.parse(result);
	});
}
...
export function getBNGROUPList(){
	return checkPyScriptReady(() => {
		const py_get_bn_group_list_func = pyscript.interpreter.globals.get('py_get_boundary_group_list');
		const result = py_get_bn_group_list_func();
		return JSON.parse(result);
	});
}