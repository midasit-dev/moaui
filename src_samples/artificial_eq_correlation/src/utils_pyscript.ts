...
export function calCorrelation(THFC_List:any, THFC_Data: any) {
	return checkPyScriptReady(() => {
		const py_cal_correlation_func = pyscript.interpreter.globals.get('calculate_correlation');
		const result = py_cal_correlation_func(JSON.stringify(THFC_List), JSON.stringify(THFC_Data));
		return JSON.parse(result);
	});
}